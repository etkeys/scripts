...
if [ -z "${DOCKER_OPENWEBUI_CONTAINER}" ]; then
    echo "DOCKER_OPENWEBUI_CONTAINER is not set. Using default: open-webui"
    DOCKER_OPENWEBUI_CONTAINER="open-webui"
fi

docker cp "${DOCKER_OPENWEBUI_CONTAINER}:/app/backend/data/webui.db" "${TEMP_DIR}/webui.db"
if [ $? -ne 0 ]; then
